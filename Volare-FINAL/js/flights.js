    ...
    try {
      ...
      if (data.data && data.data.length > 0) {
        data.data.forEach((offer) => {
          ...
          const bookingUrl = `https://www.skyscanner.com/transport/flights/${origin}/${destination}/${departureDate.replaceAll('-', '')}/`;

          flightInfo.innerHTML = `
            <a href="${bookingUrl}" target="_blank" class="flight-link" style="text-decoration: none; color: inherit;">
              <h3><strong>Airline:</strong> ${offer.validatingAirlineCodes.join(", ")}</h3>
